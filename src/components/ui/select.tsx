...
>(({ className, children, error, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      "flex h-9.5 items-center justify-between whitespace-nowrap rounded-sm border-2 px-3 py-2 text-sm ring-offset-background data-placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-ring disabled:cursor-not-allowed disabled:opacity-50 [&>svg]:hidden",
      error
        ? "border-semantic-red1 focus:ring-semantic-red1 "
        : "border-neutral-gray2 focus:ring-primary-main ",
      className ? className : ""
    )}
    {...props}
  >
    ...
    <SelectPrimitive.Icon asChild>
      <ChevronDown className="ml-2 text-neutral-black w-4 h-4" />
...
>(({ className, ...props }, ref) => (
  <SelectPrimitive.Separator
    ref={ref}
    className={cn("-mx-1 my-1 h-px bg-muted",    className ? className : ""
   )}
    {...props}
  />
));
